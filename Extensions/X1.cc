...
//
// ********************************************************************
// *  TOPAS X1 Custom component of DCS                                *
// *                                                                  *
...
X1::X1(TsParameterManager* pM, TsExtensionManager* eM, TsMaterialManager* mM, TsGeometryManager* gM,
...
G4VPhysicalVolume* X1::Construct()
{
	BeginConstruction();
	...
	// Compute theta, medial edge, and then trimmer centroid location
        G4cout << "Medial edge for X1 at Isoplane = " << (Xpos-TrimmerOffset) << "mm" << G4endl;
	G4double XMedial = (Xpos-TrimmerOffset)* (1 - (XAD/MADx));
	G4cout << "Medial edge for X1 at Trimmer Plane = " << XMedial << "mm" << G4endl;
	G4double Theta = atan(XMedial/(MADx-XAD)); // Value in radians
	G4cout << "Computed theta for X1 = " << Theta * 180 / PI << "deg" << G4endl;
	G4double XTrimmerCenter = XMedial - (TrimmerHalfWidth/cos(Theta));
	//G4cout << "Centroid location for X1 = " << XTrimmerCenter * 0.1 << "cm" << G4endl;
	...
	{
	G4ThreeVector* pos = new G4ThreeVector(-1.0*DefaultPositive,0.0,XAD);
	G4RotationMatrix* rot = new G4RotationMatrix();
	rot->rotateX(0.0); rot->rotateY(-1.0*DefaultRotation); rot->rotateZ(0.0);
	fEnvelopePhys = CreatePhysicalVolume(fName,fEnvelopeLog,rot,pos,fParentVolume);
	}
